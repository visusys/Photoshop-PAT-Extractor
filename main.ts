		...
		this.res.patterns.forEach((item, patternIndex) => {
			...
			for (const key in channels) {
				if (channels.hasOwnProperty(key) && channels[key]) {
					...
					fs.writeFileSync("C:/Users/futur/Desktop/pat-parser-master/src/pat" + "_"+patternIndex + "_" + key + "_" + item.name + "_" + item.memoryList.channels[0]?.compressionMode + "_" + item.memoryList.channels[0]?.pixelDepth + "_" + "alpha-" + item.hasAlphaChannel + "_" + item.colorMode + "_.png", result,{encoding:"binary"});
